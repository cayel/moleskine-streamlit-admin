...
def load_movies(selected_user_id):
    if os.path.exists('moleskine.db'):
        ...
        movies_query = "SELECT id, utilisateur_id, director, title, rating, date, cinema, imageUrl FROM movies WHERE utilisateur_id = ?"
        filtered_movies = cursor.execute(movies_query, (selected_user_id,)).fetchall()

        df_filtered_movies = pd.DataFrame(filtered_movies, columns=['id', 'utilisateur_id', 'director', 'title', 'rating', 'date', 'cinema', 'imageUrl'])
